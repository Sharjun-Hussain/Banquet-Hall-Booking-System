...
exports.FetchPendingBookings = async (req, res, next) => {
    ...
    const Halldata = await hallModel.findOne(hallid)
    res.status(200).send({
        success: "true",
        Bookingdata,
        Halldata
...
exports.FetchAcceptedBookings = async (req, res, next) => {
    ...
    hallid = Bookingdata.HallID
    const Halldata = await hallModel.findOne(hallid)
    // const Halldata = await hallModel.findOne(hallid, { HallName: 1, _id: 0 }) oroginal
    res.status(200).send({
        success: "true",
        Bookingdata,
        Halldata
